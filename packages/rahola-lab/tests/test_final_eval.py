...
def test_development_seed_api_still_refuses_reserve() -> None:
    with pytest.raises(ReserveBlockError):
        seeds_for(SeedBlock.RESERVE, 1)
...
def test_final_evaluation_refuses_dirty_tree_before_attestation(
    ...
    assert not (tmp_path / "results" / "final_reserve_attestation.json").exists()
