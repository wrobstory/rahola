...
class GammaRatePrior:
    ...
    @classmethod
    def from_mean(cls, mean_rate: float, strength: float) -> GammaRatePrior:
        if not np.isfinite(mean_rate) or mean_rate <= 0.0:
            raise ValueError("prior mean rate must be finite and positive")
        return cls(shape=float(strength), rate=float(strength) / mean_rate)
...
def exponential_rate_mle(
    severities_u: NDArray[np.floating], *, quantile: float
) -> float:
...
def estimate_exponential_tail(
    ...
    *,
    ...
    critical_probability = (len(exceedances) / len(values)) * predictive
