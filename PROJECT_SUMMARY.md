...
## 8. August 2026 v0.2 methodology-freeze addendum

The v0.2 repair program accepted the external review in full without rewriting the historical
record. It corrected asymmetric alarm scaling, made the critical-roll-rate margin explicitly
two-sided, fixed LSTM epoch sampling, made the roll-band statistic one-sided, removed hardcoded
natural periods, strengthened campaign invariants, rejected ambiguous nonstationary restarts, and
added the missing MIT license. Episode and AUC uncertainty now resample complete trajectories and
recompute the complete scoring logic. Exact binomial intervals remain only for genuine event
counts.

Three detector preprocessing modes were frozen before rescoring. Physics-adjacent methods use
configured nondimensional units. The fixed-window past-only transform is primary for the CNN and
XGBoost, while the cumulative-online transform is reported separately as a detector system with
full-history normalization state. In pooled D1, the primary CNN reaches 91.00% sensitivity at
13.409 false episodes per hour; cumulative-online reaches 93.83% at 16.451 per hour. Neither mode
produces a 90%-sensitivity threshold across every held-out family.

The forcing field is now defined independently of integration resolution, with a cutoff at 40
times the natural frequency. A predeclared one-percentage-point prevalence rule required versioned
replacement data for the $`\gamma=7,15,30`$ bandwidth campaigns. Other historical campaigns remain
the selected data. The initial ratio-4 stress run remains an audit artifact, not a competing
analysis.

The longer 900-second step campaign resolves the ambiguity in the historical D5 geometry. Every
scored history begins after the 300-second sea-state transition, and every outcome horizon is
complete. The preregistered prediction was retained: no motion-only method exceeded
orientation-independent AUC 0.556, below the 0.58 trigger. This supports the narrower claim that
tested motion-only scores remain near chance after a complete post-transition wash-in. It does not
identify the cause or establish a universal information ceiling.

The v0.2 record also states each method's information set. The danger margin is a
known-configuration physics comparator, and the engineered XGBoost model has
configuration-assisted features. C1/C2 restart comparisons know the configured sea and replace the
realized future with common-random-number independent rollouts; they still are not Bayes-optimal
motion-history ceilings. On the v0.2 stratified sample, fixed-window/cumulative CNN AUC is
0.652/0.622 and fixed-window/cumulative XGBoost AUC is 0.723/0.768. Exact-state and filtered-state
restart AUC is 0.850 and 0.486; the clock-only comparator reaches 0.656. These results strengthen
the protocol-time warning and do not define an information ordering.

The complete side-by-side record is in `RESULTS_v02.md`. No reserve,
reserve-2, paper draft, explainer, or historical result artifact was changed.
